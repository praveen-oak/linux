        ...
        if (arguments["list"] | arguments["savelist"]):
           ...
           #well, this isn't the most optimal.
            ...
            for sourcefile in matches:
                        ...
                        if (arguments["patch"]):
                            for patch in data:
                                ...
                                for syscall in patch["calls"]:
                                    syscall_replacer = re.compile("(SYSCALL_DEFINE[0-9]\((?P<call>" + syscall + ")(,.*?)?\)([\s\r\n])*?{(\s)?)(//AUTOGENERATED STUB INSERTED BY scripts\/syscall_tool.py DO NOT MANUALLY REMOVE \(USE SCRIPT TO REMOVE\).*?//END AUTOGENERATED STUB\n)?", re.MULTILINE | re.DOTALL)
                ...
                if overwrite:
                    with open(sourcefile, "w") as sourcefile_file:
                        if (arguments["--verbose"]):
                            print("Patching " + sourcefile)
