...
{
//AUTOGENERATED STUB INSERTED BY scripts/syscall_tool.py DO NOT MANUALLY REMOVE (USE SCRIPT TO REMOVE)
	int __ignoreme = speculate_away_and_wait();
...
ssize_t do_sync_write2(struct file *filp, const char __user *buf, size_t len, loff_t *ppos)
{
	struct iovec iov = { .iov_base = (void __user *)buf, .iov_len = len };
	...
	if (-EIOCBQUEUED == ret) {
		printk("queue queue queue queue queue queue\n");
		add_kiocb(kiocb);
//		ret = wait_on_sync_kiocb(&kiocb);

		*ppos += len; //kiocb.ki_pos;
		return len;
	} else {
		speculate_remove_iowait();
		*ppos = kiocb->ki_pos;
		kfree(kiocb);
		return ret;
	}
...
ssize_t _vfs_write(struct file *file, const char *buf, size_t count, loff_t *pos, struct task_struct *tsk)
{
	int ret = 0;
	//printk("------2 %p %p %u %p\n", file, buf, count, pos);
...
//END AUTOGENERATED STUB
	...
	if (f) {
		if (0) { //activate_specu()) {
			...
			ret = count;
			file_pos_write(f, pos+count);
		} else {
			loff_t pos = file_pos_read(f);
			ret = vfs_write(f, buf, count, &pos);
			if (ret >= 0)
