...
static int nvme_queue_request(struct blk_mq_hw_ctx *hctx, struct request *rq)
{
	...
	printk("owoowowowowowowowowowowwowowowowowowowow\n");
	/* TODO: I don't see how this can fail */
	if (!nvmeq)
		return BLK_MQ_RQ_QUEUE_ERROR;
...
static int nvme_init_io_hctx(struct blk_mq_hw_ctx *hctx, void *data,
			  unsigned int i)
{
	struct nvme_dev *dev = data;
	struct nvme_queue *nvmeq = dev->queues[i + 1];
	printk("larlalrallarlarlrlarlrlarlrlrlarllarlalralralrlalralrla\n");
	nvmeq->hctx = hctx;
	hctx->driver_data = nvmeq;
	return 0;
}
...
static int nvme_init_io_request(void *data, struct request *rq,
				unsigned int hctx_idx, unsigned int rq_idx,
				unsigned int numa_node)
{
	struct nvme_dev *dev = data;
...
static struct blk_mq_ops nvme_mq_ops = {
	...
	.alloc_hctx		= nvme_alloc_hctx,
	.free_hctx		= nvme_free_hctx,
	.init_hctx		= nvme_init_io_hctx,
	.init_request		= nvme_init_io_request,
	.timeout		= nvme_timeout,
};
...
static int nvme_configure_admin_queue(struct nvme_dev *dev)
{
	...
	if (!nvmeq) {
		...
		dev->admin_tags.driver_data = nvmeq;
...
static int nvme_dev_start(struct nvme_dev *dev)
{
	...
	printk("2larlalrallarlarlrlarlrlarlrlrlarllarlalralralrlalralrla\n");
	result = nvme_setup_io_queues(dev);
	printk("3larlalrallarlarlrlarlrlarlrlrlarllarlalralralrlalralrla\n");
